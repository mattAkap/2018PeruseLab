...
public class Path implements Crossable {
	...
	public List<Point> getIntersectionsPath(Crossable aPath) {
		...
		for (int i = 0; i < this.points.size() - 1; i++) {
			...
			Line2D line1 = new Line2D.Double(firstPoint.getX(), firstPoint.getY(), secondPoint.getX(),
					secondPoint.getY());
			...
			if (result) {
				...
				x2 = secondPoint.getX();
				y2 = secondPoint.getY();
				x3 = ((Line) aPath).getStart().getX();
				y3 = ((Line) aPath).getStart().getY();
				x4 = ((Line) aPath).getEnd().getX();
	...
	public boolean isCrossingPath(Crossable crossable) {
		return getIntersectionsPath(crossable) != null;
	}
	 ...
	 * @return boolean
	 */
	public boolean isNotCrossingPath(Crossable crossable) {
		return (getIntersectionsPath(crossable) == null);
	}
